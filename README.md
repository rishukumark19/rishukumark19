<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=rishukumark19&show_icons=true&theme=onedark&hide_border=true" alt="Rishu's GitHub Stats" height="150" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=rishukumark19&theme=onedark&hide_border=true" alt="Rishu's GitHub Streak" height="150" />
</p>

# 🚀 Rishu Kumar — Developer Portfolio
<p align="center">
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB.svg?style=for-the-badge&logo=python&logoColor=white"/>
  <img alt="FastAPI" src="https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi"/>
  <img alt="Tailwind CSS" src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwind-css&logoColor=white"/>
  <img alt="HTMX" src="https://img.shields.io/badge/HTMX-3498DB?style=for-the-badge&logo=htmx&logoColor=white"/>
  <img alt="Django" src="https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white"/>
</p>

### Core Learning Areas Showcased:
* **Python Backend Ecosystem:** FastAPI, Django, Pydantic.
* **Modern Frontend:** Mastering Tailwind CSS for production-ready design.
* **Workflow:** Automation, AI/Prompt Engineering, Git/GitHub.
* **Creative:** Video editing and motion graphics (DaVinci Resolve).

---

## 🎓 Education

**National Institute of Technology Karnataka (NITK), Surathkal**  
B.Tech in Electronics and Communication Engineering  
*Batch of 2025*

**GGCET St. Xavier's, Dhanbad**  
- Class XII: 95% (2019-2021)
- Class X: 85.4% (2017-2019)

---

## 🛠️ Technical Skills

### Backend Development
- **Django:** MVT architecture, Function-Based Views (FBVs), Class-Based Views (CBVs)
- **ORM:** Django Models, complex database schemas, relationships
- **Database:** SQLite, migrations, schema management, relational design
- **API Development:** FastAPI, RESTful services

### Tools & Technologies
- **Version Control:** Git, GitHub (source control and collaboration)
- **Cloud & Deployment:** Google Cloud Platform (GCP)
- **Environment Management:** Python venv for isolated environments
- **AI & Automation:** Prompt Engineering, AI-driven solutions
- **Data Modeling:** One-to-many relationships, database design

### Frontend & Design
- **Frameworks:** Tailwind CSS, HTMX
- **Web Development:** Responsive design, user-friendly interfaces

---

## 💼 Projects & Experience

### President, SAE NITK
- Led multiple technical projects, workshops, and outreach programs
- Significantly improved student participation and industry collaborations
- Organized national-level events bridging academia and industry

### Publicity Head, NSS NITK
- Designed and executed comprehensive digital marketing campaigns
- Achieved 50% rise in online engagement and volunteer participation
- Managed social media accounts with analytics-driven approaches
- Enhanced visibility through strategic content

### Freelance Marketing & Social Media Consultant
- Assisted businesses and individuals in enhancing digital presence
- Increased brand awareness and optimized engagement through tailored strategies
- Achieved 40% boost in social media reach and user interaction

### Web Enthusiasts Club, NITK
- Contributed to front-end and back-end web development projects
- Designed responsive, user-friendly web applications for college events and initiatives

### Django Web Applications
- Developed and deployed three distinct web applications using Django's MVT architecture
- Implemented clean URL structures with Django's URL Dispatcher
- Built relational database designs connecting posts with User models
- Configured SQLite databases and maintained schema consistency

---

## 🌟 Extracurricular & Personal Interests

### Automation & AI
- Passionate about AI-driven solutions and workflow automation
- Integrating intelligent tools into development pipelines
- Skilled in prompt engineering for coding assistance

### Social Service
- **NSS Volunteer:** Organized awareness drives, social initiatives, and leadership programs

### Creative Skills
- **Videography & Editing:** Proficient in DaVinci Resolve, Adobe Premiere Pro, and After Effects
- Creating engaging media content and motion graphics

### Fitness & Sports
- Enthusiastic about gym training, volleyball, and endurance activities

### Other Interests
- Technology exploration and digital marketing
- Business automation and content creation
- Public speaking and data science

---

### 📬 Let's Connect!

<a href="mailto:rishukumar.work@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
<a href="https://www.linkedin.com/in/YOUR_LINKEDIN_PROFILE" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
<a href="https://x.com/rishukumark19" target="_blank"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" /></a>

**Project Status: Open for Internships/Collaborations.**

<p align="center">
Made with ☕ and curiosity by Rishu Kumar.
</p>
